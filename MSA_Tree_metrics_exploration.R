...
server <- function(input, output, session) {
  ...
  # Auto-uncheck default files when uploading custom file
  observe({
    if (!is.null(input$upload_file)) {
      updateCheckboxGroupInput(session, "use_default_files", selected = character(0))
    }
  })
  
  # Ensure only one default file can be selected at a time
  observe({
    if (length(input$use_default_files) > 1) {
      # If multiple selected, keep only the last one
      latest_selection <- input$use_default_files[length(input$use_default_files)]
      updateCheckboxGroupInput(session, "use_default_files", selected = latest_selection)
    }
  })
  
  # Optimized reactive expression to read data using data.table
  current_data <- reactive({
    # Check if custom file is uploaded
    if (!is.null(input$upload_file)) {
      dt <- tryCatch({
        # Use fread for much better memory efficiency and speed
    ...
    # Only load one dataset at a time from default files using fread
    if ("dna" %in% input$use_default_files) {
      if (file.exists("metrics.GTR.csv")) {
        dt <- tryCatch({
          fread("metrics.GTR.csv")
        }, error = function(e) {
  ...
  # Optimized variable choices update using data.table operations
  observe({
    ...
    # Vectorized filtering of choices based on available numeric columns
  ...
  simulation_data <- eventReactive(input$apply_simulation, {
    ...
    # Validate that required model components have corresponding data
    missing_vars <- character(0)
    
    if ("base_model" %in% input$model_components && input$var_type == "dna") {
      dna_rate_vars <- c("rateAC", "rateAG", "rateAT", "rateCG", "rateCT")
      missing_rates <- dna_rate_vars[!dna_rate_vars %in% names(df)]
    ...
    if ("frequencies" %in% input$model_components) {
      ...
      missing_freqs <- freq_vars[!freq_vars %in% names(df)]
      if (length(missing_freqs) > 0) {
        missing_vars <- c(missing_vars, missing_freqs)
      }
    }
    
    if ("gamma" %in% input$model_components && !"GammaShape_alpha" %in% names(df)) {
      missing_vars <- c(missing_vars, "GammaShape_alpha")
    }
    
    if ("invariant" %in% input$model_components && !"proportion_invariant" %in% names(df)) {
      missing_vars <- c(missing_vars, "proportion_invariant")
    }
    
    if (length(missing_vars) > 0) {
      showNotification(
        paste("Missing required variables for selected model components:", paste(missing_vars, collapse = ", ")),
